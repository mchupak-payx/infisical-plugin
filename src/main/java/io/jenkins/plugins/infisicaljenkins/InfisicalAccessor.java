...
public class InfisicalAccessor implements Serializable {
    ...
    public Map<String, String> fetchInfisicalSecrets(
            Run<?, ?> run,
            PrintStream logger,
            EnvVars envVars,
        ...
        InfisicalConfiguration config = pullAndMergeConfiguration(run, initialConfiguration);
            ...
            for (InfisicalSecretValue secretValue : infisicalSecret.getSecretValues()) {
                if (secretValue.getIsRequired()) {
                    if (!environmentVariables.containsKey(secretValue.getInfisicalKey())) {
                        throw new InfisicalPluginException(String.format(
                                "Secret %s is required but not found in Infisical in path %s",
    ...
    private InfisicalCredential getCredential(
            @SuppressWarnings({"rawtypes"}) Run build, InfisicalConfiguration config) {
        if (Jenkins.getInstanceOrNull() != null) {
            ...
            List<InfisicalCredential> credentials = CredentialsProvider.lookupCredentials(
                    InfisicalCredential.class, build.getParent(), ACL.SYSTEM, Collections.emptyList());
            InfisicalCredential credential = CredentialsMatchers.firstOrNull(credentials, new IdMatcher(id));
    ...
    public static InfisicalConfiguration pullAndMergeConfiguration(
            Run<?, ?> build, InfisicalConfiguration buildConfiguration) {
